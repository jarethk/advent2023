...
const zipArrs = (a, b) =>
    a.map((el, idx) => [el, b[idx]]);

const compareZip = (a) =>
    a.every(element => element[0] === element[1]);

const diffZip = (a) =>
    a.flatMap((element) => zipArrs(element[0].split(""), element[1].split("")))
        .map(element => element[0] === element[1] ? 0 : 1)
        .reduce((a, b) => a + b);
...
function scoreBlock2(block) {
    ...
    for (let l1 = 1; l1 < block.length; l1++) {
        ...
        dl(`Checking slice ${l1}: ${part1}`)
        dl(`      vs slice ${l1}: ${part2}`)
        if (diffZip(zipArrs(part1, part2)) == 1) {
